...
Vector callTranslation(const Pose3& p)
{
  return p.translation().vector(); 
}

Vector callIntegrateError(const LieVector& Pij, const LieVector& Vij, const LieVector& theta_ij, const Vector3& mAcc, const Vector3& mOmega,  double deltaT)
{
  Rot3 Rij = Rot3::Expmap(theta_ij.vector()); 
  Vector3 nPij = Pij.vector() + Vij.vector()*deltaT + 0.5*Rij.matrix()*mAcc*deltaT*deltaT; 
  Vector3 nVij = Vij.vector() + Rij.matrix() * mAcc * deltaT; 
  Vector3 theta_incr = mOmega * deltaT; 
  Rot3 Rincr = Rot3::Expmap(theta_incr); 
  Rot3 nRij = Rij * Rincr; 
  Vector3 ntheta_ij = Rot3::Logmap(nRij);

  Vector ret(9); ret << nPij, nVij , ntheta_ij; 
  return ret;
}
...
TEST(ImuPreIngrete, Jacobian)
{
  Vector3 gravity; gravity << 0, 0, 9.81; 
  Pose3 x1(Rot3::RzRyRx(M_PI/3, M_PI/4, 0), Point3(1.0, 0, 0)); 
  imuBias::ConstantBias bias; 
  Vector3 measuredOmega; measuredOmega << M_PI/100, M_PI/20, 0; 
  Vector3 measuredAcc = x1.rotation().unrotate(-Point3(gravity)).vector(); 

  double deltaT = 0.03; 
  ImuFactor::PreintegratedMeasurements pre_ini_data(bias, Matrix3::Zero(), Matrix3::Zero(), Matrix3::Zero()); 
  pre_ini_data.integrateMeasurement(measuredAcc, measuredOmega, deltaT); 

  LieVector Pij(pre_ini_data.deltaPij); 
  LieVector Vij(pre_ini_data.deltaVij); 
  Rot3 deltaRij = pre_ini_data.deltaRij; 
  Vector3 theta_ij_lie = Rot3::Logmap(deltaRij);
  LieVector theta_ij(theta_ij_lie);

  Matrix H1e = numericalDerivative11<LieVector>(
      boost::bind(&callIntegrateError, _1, Vij, theta_ij, measuredAcc, measuredOmega, deltaT), Pij);
  Matrix H2e = numericalDerivative11<LieVector>(
      boost::bind(&callIntegrateError, Pij, _1, theta_ij, measuredAcc, measuredOmega, deltaT), Vij); 
  Matrix H3e = numericalDerivative11<LieVector>(
      boost::bind(&callIntegrateError, Pij, Vij, _1, measuredAcc, measuredOmega, deltaT), theta_ij); 
  

  cout<<"H1e: "<<endl<<H1e<<endl; 
  cout<<"H2e: "<<endl<<H2e<<endl;
  cout<<"H3e: "<<endl<<H3e<<endl;
...
TEST(ImuFactor, DE)
{
  ...
  Pose3 x2(Rot3::RzRyRx(M_PI/3, M_PI/4, 0), Point3(1.5, 0, 0)); 
  LieVector v2(v1); 

  // Measurements 
  Vector3 gravity; gravity << 0, 0, 9.81; 
  Vector3 omegaCoriolis; omegaCoriolis << 1, 0, 0; 
  Vector3 measuredOmega; measuredOmega << 0 , 0, 0; 
  Vector3 measuredAcc = x1.rotation().unrotate(-Point3(gravity)).vector(); // -Rt 
  ...
  // create factor 
  ImuFactor factor(X(1), V(1), X(2), V(2), B(1), pre_ini_data, gravity, omegaCoriolis); 
  
  // Expected Jacobians
  Matrix H1e = numericalDerivative11<Pose3>(
      boost::bind(&callEvaluateError, factor, _1, v1, x2, v2, bias), x1);
  Matrix H1etop3 = H1e.topRows(3); 
  // cout<<"predicted H1etop3: "<<H1etop3<<endl;

  Vector3 w; w << M_PI/100, M_PI/3, 2; 
  double theta = w.norm(); 
  Point3 n(w/theta); 
  Rot3 R = Rot3::rodriguez(n.vector(), theta); 
  
  cout<<"consutuct a rotation matrix: "<<endl<<R.matrix()<<endl;
  Matrix3 n_hat = skewSymmetric(n.vector()); 
  cout<<"n_hat = : "<<endl<<n_hat<<endl; 
  Matrix3 R_n_hat = R.matrix() * n_hat; 
  cout<<"R_n_hat = : "<<endl<<R_n_hat<<endl;

  Matrix Hte = numericalDerivative11<Pose3>(callTranslation, x1); 
  cout<<"predicted Hte: "<<endl<<Hte<<endl; 
  cout<<"x1.R: "<<endl<<x1.rotation().matrix()<<endl;
