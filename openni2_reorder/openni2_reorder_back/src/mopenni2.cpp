...
bool MOpenni2::getFrame(cv::Mat& rgb, cv::Mat& depth)
{
	...
	if (m_colorFrame.isValid())
	{
		...
		memcpy(m_rgb8u_.data,pColor,m_depthFrame.getHeight()*m_depthFrame.getWidth()*3);
...
double MOpenni2::getRosTime()
{
	...
	if ( ( start_sec == 0 ) && ( start_nsec == 0 ) )
	{
		...
		if (cpu_freq.QuadPart == 0) {
			...
			printf("This windows platform does not support the high-performance timing api.\n");
			return -1;
		}
	...
	uint32_t sec = sec_sum;
	uint32_t nsec = nsec_sum;
