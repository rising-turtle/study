...
public:
	...
	ostream& output(ostream& out)
	{
		...
		out<<m_coords[0]<<" "<<m_coords[1]<<" "<<m_coords[2]<<" "<<roll<<" "<<pitch<<" "<<yaw<<endl;
		return out;
	}
	void input_f(istream& in_)
	{
		char tmpchar[100];
		double roll_,pitch_,yaw_;
		in_.getline(tmpchar,100);
		sscanf(tmpchar,"(x,y,z)(%lf,%lf,%lf)",&m_coords[0],&m_coords[1],&m_coords[2]);
		in_.getline(tmpchar,100);
		sscanf(tmpchar,"(r,p,y)(%lf,%lf,%lf)",&roll_,&pitch_,&yaw_);
		this->roll=D2R(roll_);
		this->pitch=D2R(pitch_);
		this->yaw=D2R(yaw_);
		rebuildRotationMatrix();
		FromEular2Quaternions();
	}
