  ...
  : QScrollArea( parent )
{
  ...
  imagewidget = new ImageMapWidget(this); // this is the main widget for ScrollArea
  setWidgetResizable(1); // change with the size change
...
void
MapPainter::update(CMap2D* map )
{
	imagewidget->updateMap(map);
/*
...
void
MapPainter::refresh( CMap2D* map )
{
  viewport()->repaint( TRUE );
}
void MapPainter::receMap(CMap2D* map){
	update(map);
}
/*
void
MapPainter::showscan( CMap2D* map, float* lsens, double maxrange )
{
  int                   i;
...
/*
void
MapPainter::drawrobot( CMap2D* map, ivector2d  pos, int color )
{
...
MapPainter::doPaint(  ) {
  ...
  resize(imagewidget->m_pm->width(),imagewidget->m_pm->height());
